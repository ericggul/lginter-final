...
function readEnvConfig() {
  ...
  // Remote mode keys per user request
  ...
  const remoteAccessToken = process.env.HUE_ACCESS_TOKEN || ""; // not used, refresh token drives flow
...
export async function initHue(configOverride = undefined) {
  ...
  if (cfg.remoteEnabled) {
    console.log("[hue] init remote mode enabled");
    if (!cfg.username || !cfg.remoteClientId || !cfg.remoteClientSecret || !cfg.remoteRefreshToken) {
      console.warn("[hue] remote missing creds", {
        hasUsername: !!cfg.username,
        hasClientId: !!cfg.remoteClientId,
        hasClientSecret: !!cfg.remoteClientSecret,
        hasRefresh: !!cfg.remoteRefreshToken,
      });
      return {
        ok: false,
        message: "Missing remote creds (HUE_USERNAME, HUE_CLIENT_ID/SECRET/REFRESH_TOKEN)",
      };
    }
    try {
      const api = await v3.api.createRemote(
        cfg.username,
        cfg.remoteClientId,
        cfg.remoteClientSecret,
        cfg.remoteRefreshToken
      );
      hueApi = api;
      console.log("[hue] remote connected");
      return { ok: true, message: "Hue remote connected" };
    } catch (err) {
      console.error("[hue] remote connect failed", err?.message || err);
      return { ok: false, message: err?.message || "Hue remote connect failed" };
    }
  }
