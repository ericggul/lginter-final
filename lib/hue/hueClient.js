...
function parseColor(color) {
  ...
  throw new Error("Unsupported color format. Use #RRGGBB or rgb(r,g,b)");
}
...
function resolveTargets(targets) {
  ...
  const t = targets || {};
  const lightIds = Array.isArray(t.lightIds) && t.lightIds.length ? t.lightIds.map((n) => Number(n)) : base.lightIds;
  const groupId = t.groupId != null ? Number(t.groupId) : base.groupId;
  return { lightIds, groupId };
}

export async function setLightColor(options) {
  const { color, brightness, transitionMs, targets } = options || {};
  const cfg = cachedConfig || readEnvConfig();
  if (!cfg.enabled) return { ok: false, disabled: true, error: "Hue disabled" };
  if (!hueApi) {
    const r = await initHue(cfg);
    if (!r.ok) return { ok: false, error: r.message, disabled: r.disabled };
  }
  ...
  const state = new LightState().on().xy(x, y).brightness(briFinal);
  ...
  const apply = async () => {
    if (groupId != null && Number.isFinite(groupId)) {
      return hueApi.groups.setGroupState(groupId, state);
    }
    if (Array.isArray(lightIds) && lightIds.length > 0) {
      const results = [];
      for (const id of lightIds) {
        // eslint-disable-next-line no-await-in-loop
        results.push(await hueApi.lights.setLightState(Number(id), state));
      }
      return results;
    }
    // Fallback: all lights in all-lights group 0 (not always supported)
    return hueApi.groups.setGroupState(0, state);
  };

  try {
    const result = await apply();
    return { ok: true, applied: true, result, xy: { x, y }, bri: briFinal };
  } catch (e) {
    // one retry
    try {
      const result = await apply();
      return { ok: true, applied: true, result, xy: { x, y }, bri: briFinal, retried: true };
    } catch (err) {
