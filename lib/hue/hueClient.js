...
async function ensureRemoteReady(cfg) {
  syncRemoteAuthFromConfig(cfg);
  if (!cfg?.username) return { ok: false, error: "Missing HUE_USERNAME (required for remote routing)" };
...
async function remoteRequest(cfg, path, { method = "GET", body } = {}) {
  ...
  const base = `https://api.meethue.com/bridge/${encodeURIComponent(cfg.username)}`;
  ...
  if (cfg.remoteEnabled) {
    const applyRemoteLights = async (ids) => {
      ...
      for (const id of ids) {
        ...
        else errors.push({ id: Number(id), ok: false, error: resp.error, raw: resp.raw });
      }
      if (results.length === 0) throw new Error(errors[0]?.error || "No lights were updated");
    ...
    const applyRemoteGroup = async (gid) => {
      ...
      if (!resp.ok) throw new Error(resp.error || "Remote group apply failed");
      return resp.data;
    };

    const apply = async () => {
      if (groupId != null && Number.isFinite(groupId)) return await applyRemoteGroup(groupId);
      if (Array.isArray(lightIds) && lightIds.length > 0) return await applyRemoteLights(lightIds);
