...
function readEnvConfig() {
  ...
  const remoteEnabled = String(process.env.HUE_REMOTE_ENABLED || "").toLowerCase() === "true";
  const remoteClientId = process.env.HUE_REMOTE_CLIENT_ID || process.env.HUE_CLIENT_ID || "";
  const remoteClientSecret =
    process.env.HUE_REMOTE_CLIENT_SECRET || process.env.HUE_CLIENT_SECRET || "";
  const remoteRefreshToken =
    process.env.HUE_REMOTE_REFRESH_TOKEN || process.env.HUE_REFRESH_TOKEN || "";
  // Support both:
...
export async function initHue(configOverride = undefined) {
  ...
  if (cfg.remoteEnabled) {
    if (!cfg.username || !cfg.remoteClientId || !cfg.remoteClientSecret || !cfg.remoteRefreshToken) {
      return {
        ok: false,
        message: "Missing remote creds (HUE_USERNAME, HUE_REMOTE_CLIENT_ID/SECRET/REFRESH_TOKEN)",
      };
    }
